...
2. With Illustrator or any vector editing program:
   ...
   2. Place just one (at most two) pages from the example PDF into the layer. Make sure any artwork is center-aligned and fitted _exactly_ to an area within a **40px margin all-around**.
   3. Export the asset as a **PNG image**.
3. Place the exported thumbnail in `public/thumbnails`.
