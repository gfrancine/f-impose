...
import { drawPageWithTransform, Vec2 } from "../utils";
...
async function impose(srcPdf: PDFDocument) {
  ...
  for (let i = 0; i < srcPages.length; i++) {
    ...
    if (i % 2 === 1) {
      drawPageWithTransform(outPage, srcPages[i], new Vec2(w / 2, h / 2), {
        rotateDeg: 180,
      });
    } else {
      outPage.drawPage(srcPages[i], { x: 0, y: 0, width: w, height: h });
    }
  }
